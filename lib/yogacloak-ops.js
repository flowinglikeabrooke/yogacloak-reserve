...
import { adminUserForToken, getAdminSession, requireCsrf } from './admin-auth.js';
...
function requireAdmin(req, res) {
  ...
  if (!allowed.length || !allowed.includes(provided)) {
    ...
    req.adminUser = adminUserForToken(provided) || {
      id: provided === cronToken ? 'system-cron' : 'owner-token',
      name: provided === cronToken ? 'System cron' : 'Owner token',
      email: '',
      role: 'founder'
  };
  return true;
...
function requireOwner(req, res) {
  ...
  if (session) {
    req.adminUser = session;
    if (!requireCsrf(req, res)) return false;
    if (['founder', 'owner'].includes(session.role)) return true;
    res.status(403).json({ error: 'Owner access required.' });
    return false;
  }

  const tokenUser = adminUserForToken(provided);
  if (tokenUser) {
    req.adminUser = tokenUser;
    if (['founder', 'owner'].includes(tokenUser.role)) return true;
    res.status(403).json({ error: 'Owner access required.' });
    return false;
  }

  if ([token, cronToken].filter(Boolean).includes(provided)) {
    req.adminUser = {
      id: provided === cronToken ? 'system-cron' : 'owner-token',
      name: provided === cronToken ? 'System cron' : 'Owner token',
      email: '',
...
function requireFounder(req, res) {
  ...
  if (session) {
    req.adminUser = session;
    if (!requireCsrf(req, res)) return false;
    if (session.role === 'founder') return true;
  ...
  if (tokenUser) {
    req.adminUser = tokenUser;
    if (tokenUser.role === 'founder') return true;
    res.status(403).json({ error: 'Founder access required.' });
    return false;
  }
...
async function redisRateLimit({ key, maxRequests, windowSeconds }) {
  const config = redisRateLimitConfig();
...
function localRateLimit({ key, maxRequests, windowSeconds }) {
  ...
  if (store[key].length >= maxRequests) return { allowed: false, remaining: 0 };

  store[key].push(now);
  return { allowed: true, remaining: Math.max(0, maxRequests - store[key].length) };
}

async function checkRateLimit(req, res, { maxRequests = 10, windowSeconds = 60, keyPrefix = 'default' } = {}) {
  const now = Date.now();
  const windowMs = windowSeconds * 1000;
  const bucket = Math.floor(now / windowMs);
  const key = `ratelimit:${keyPrefix}:${rateLimitId(req)}:${bucket}`;
  let result = null;

  try {
    result = await redisRateLimit({ key, maxRequests, windowSeconds });
  } catch (err) {
    if (!global._redisRateLimitWarningShown) {
      console.warn('Redis/Vercel KV rate limiting failed; using local fallback:', err.message);
      global._redisRateLimitWarningShown = true;
    }
  }

  if (!result) result = localRateLimit({ key, maxRequests, windowSeconds });
...
function requestBodyBytes(req) {
  ...
  if (req.body && typeof req.body === 'object') return Buffer.byteLength(JSON.stringify(req.body), 'utf8');
  return 0;
}

function rejectLargeRequest(req, res, maxBytes = 64 * 1024) {
  if (requestBodyBytes(req) <= maxBytes) return false;
