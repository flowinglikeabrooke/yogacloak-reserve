...
async function upsertOrUpdateBy(table, row, conflictTarget, filters) {
  try {
    const rows = await upsertRows(table, [row], conflictTarget);
...
async function upsertReservation(input = {}) {
  ...
  if (row.airtable_reservation_id) {
    reservation = await upsertOrUpdateBy('reservations', { ...row, created_at: now }, 'airtable_reservation_id', {
      airtable_reservation_id: `eq.${row.airtable_reservation_id}`
    });
  } else if (row.checkout_session_id) {
    reservation = await upsertOrUpdateBy('reservations', { ...row, created_at: now }, 'checkout_session_id', {
...
async function recordPayment(input = {}) {
  ...
  if (row.stripe_payment_intent_id) {
    payment = await upsertOrUpdateBy('payments', row, 'stripe_payment_intent_id', {
...
async function upsertPaymentMethod(input = {}) {
  ...
  return upsertOrUpdateBy('payment_methods', {
    customer_id: input.customerId,
    stripe_customer_id: clean(input.stripeCustomerId, 180),
    stripe_payment_method_id: clean(input.stripePaymentMethodId, 180),
