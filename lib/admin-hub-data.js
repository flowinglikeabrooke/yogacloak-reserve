...
async function attachCustomerRollups(customers = []) {
  ...
  try {
    const [inquiries, reservations, payments, communications, tasks] = await Promise.all([
      ...
      selectRows('reservations', { select: 'id,customer_id,status,final_balance_total,final_balance_amount,final_balance_payment_intent_id,updated_at', filters: { customer_id: filter }, limit: 1000 }),
    ...
    for (const row of reservations || []) {
      if (!row.customer_id || row.final_balance_payment_intent_id || ['converted', 'fulfilled', 'cancelled', 'canceled'].includes(lowerStatus(row.status))) continue;
...
async function loadDashboard() {
  ...
  try {
    [customers, inquiries, reservations, payments, events, communicationsPreview] = await Promise.all([
