...
function formType(fields) {
  const details = formDetails(fields);
  if (fields['SMS Opt-In'] || details.sms_opt_in) return 'SMS Opt-In';
...
async function loadCustomers({ search = '', contactStatus = '', limit = 250 } = {}) {
  ...
  const activeDatabaseCustomers = (databaseCustomers || []).filter(activeCustomer);
  const rawCustomers = await loadAirtableCustomers({ search, contactStatus, limit });
  const seen = new Set(activeDatabaseCustomers.map((row) => row.email || row.phone || row.id).filter(Boolean));
...
async function loadInquiries({ limit = 250 } = {}) {
  ...
  const activeDatabaseInquiries = (databaseInquiries || []).filter(activeInquiry);
  const rawInquiries = await loadAirtableInquiries({ limit });
  const seen = new Set(activeDatabaseInquiries.map((row) => row.metadata?.submission_id || row.raw_airtable_record_id || row.id).filter(Boolean));
  ...
  for (const rawCustomer of rawCustomers) {
    try {
      ...
      const identity = await findOrCreateCustomer({
        ...
        email: rawCustomer.email,
        phone: rawCustomer.phone,
        status: rawCustomer.status || 'lead',
        source: 'Airtable Raw Backup',
