...
import { notificationEmailsFor, notificationLabelFor } from '../admin-notifications.js';
...
async function smsOptInsSince(cutoff) {
  const tableId = process.env.AIRTABLE_SMS_OPTINS_TABLE || process.env.AIRTABLE_SMS_TABLE || TABLES.forms;
  const records = await safeList(tableId);
  return records.filter((record) => {
    const fields = record.fields || {};
    const notes = parseNotes(fields.Notes);
    const status = String(fields['SMS Status'] || notes.sms_status || '').toLowerCase();
    const type = String(fields['Form Type'] || notes.form_type || '').toLowerCase();
    const isSmsOptIn =
      fields['SMS Opt-In'] === true ||
      notes.sms_opt_in === true ||
      status === 'subscribed' ||
      type === 'sms opt-in';
    return isSmsOptIn && after(fields['Opt-In Timestamp'] || notes.opt_in_timestamp || fields['Submission Date'], cutoff);
  }).length;
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({ ok: true, sent_to: notificationLabelFor('owners'), counts: { ...counts, sms24h } });
  } catch (err) {
