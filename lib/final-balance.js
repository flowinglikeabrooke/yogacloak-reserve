...
// A final balance only exists after Stripe confirms the deposit. Anything else
// (Pending Payment, unknown/legacy statuses) is not a purchase yet.
...
function readinessForFields(fields) {
  ...
  else if (!DEPOSIT_PAID_STATUSES.has(status)) blockedReasons.push('Deposit not paid. Stripe has not confirmed this checkout.');
...
async function sendFinalBalanceNoticeForReservation(reservationId) {
  ...
  await updateRecord(TABLES.reservations, reservationId, {
    ...
    Notes: notesWith(fields, {
      final_balance_notice_sent_at: noticeSentAt,
...
async function chargeFinalBalanceReservation(reservationId, { dryRun = false } = {}) {
  const reservation = await getRecord(TABLES.reservations, reservationId);
  const fields = reservation.fields || {};
  const notes = parseNotes(fields.Notes);
  const readiness = readinessForFields(fields);
  ...
  if (readiness.notice_wait_remaining_hours > 0) {
  ...
  if (paymentIntent.id) {
    try {
      paymentIntent = await stripeRequest(`payment_intents/${encodeURIComponent(paymentIntent.id)}?expand[]=latest_charge.balance_transaction`);
    } catch (err) {
      console.warn('Could not load final-balance Stripe fee details:', err.message);
    }
  }
  ...
  try {
    ...
    if (row?.customer_id) {
      await recordPayment({
        ...
        metadata: {
          airtable_reservation_id: reservationId,
