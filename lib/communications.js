...
async function recordCommunication({ customerId, channel, direction = 'outbound', subject = '', body = '', status = 'sent', provider = '', providerId = '', metadata = {} }) {
  ...
  let row = null;
  if (providerId) {
    const existing = await selectOne('communications', {
      filters: {
        provider: `eq.${payload.provider}`,
        provider_id: `eq.${payload.provider_id}`
      }
    });
    if (existing?.id) {
      const rows = await updateRows('communications', { id: `eq.${existing.id}` }, {
        ...payload,
        created_at: existing.created_at || payload.created_at
      });
      row = rows[0] || existing;
    } else {
      row = await insertRow('communications', payload);
    }
  } else {
    row = await insertRow('communications', payload);
  }
...
async function loadCommunications() {
  ...
  const seen = new Set();
  const uniqueSmsOptins = (smsOptins || []).filter((row) => {
    const customerId = row.customer_id || row.customers?.id || '';
    const phone = normalizePhone(row.phone || row.customers?.phone || '');
    const key = customerId || phone || row.id;
    if (seen.has(key)) return false;
    seen.add(key);
    return true;
  });
  return { sms_enabled: smsEnabled(), communications, sms_optins: uniqueSmsOptins, sms_optins_total: (smsOptins || []).length };
}
