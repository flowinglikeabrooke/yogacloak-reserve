  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      parsed.forEach((item) => {
        ...
        if ((!token && !email) || item?.status === 'disabled') return;
        users.push({
...
function adminUserForEmail(email) {
  const normalized = String(email || '').trim().toLowerCase();
  if (!normalized) return null;

  const listedUser = envAdminUsers().find((user) => String(user.email || '').trim().toLowerCase() === normalized);
  if (listedUser) return publicAdminUser(listedUser);

  const founderEmail = String(process.env.OWNER_ADMIN_EMAIL || process.env.FOUNDER_EMAIL || '').trim().toLowerCase();
  if (founderEmail && normalized === founderEmail) {
    return publicAdminUser({
      id: stableId(normalized),
      email: normalized,
      name: process.env.OWNER_ADMIN_NAME || 'Brooke',
      role: 'founder'
    });
  }

  const allowedEmails = splitEmails(process.env.ADMIN_ALLOWED_EMAILS || process.env.GOOGLE_ADMIN_ALLOWED_EMAILS);
