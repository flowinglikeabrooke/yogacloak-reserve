...
function summaryRows(results) {
  return results.map((result) => {
    ...
    return `${result.reservation_record_id}: ${status} ${amount} ${reason}`.trim();
  }).join('\n');
}
    ...
    html: `
      <div style="font-family:Helvetica,Arial,sans-serif;background:#1E2320;color:#fbf8f0;padding:32px">
        <div style="max-width:680px;margin:0 auto">
          ...
          <pre style="white-space:pre-wrap;background:#151618;border:1px solid rgba(251,248,240,.12);border-radius:12px;padding:14px;color:rgba(251,248,240,.72);font-size:12px">${summaryRows(results)}</pre>
        </div>
      </div>
    `
  });
  ...
  for (const reservationId of reservationIds) {
    try {
      ...
      results.push({
        ...
        reason: result.result && !['charged', 'already_charged', 'dry_run'].includes(result.result) ? `Stripe status: ${result.status || result.result}` : ''
      });
    } catch (err) {
      results.push({
        reservation_record_id: reservationId,
        status: dryRun ? 'skipped' : 'failed',
        reason: err.message || 'Could not charge final balance.'
      });
    }
  }
