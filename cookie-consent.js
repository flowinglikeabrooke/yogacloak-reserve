...
var googleMeasurementId='G-VYYG77LMLD';
var analyticsLoaded=false;
function readChoice(){
var match=document.cookie.split(';').map(function(item){return item.trim()}).find(function(item){return item.indexOf(cookieName)===0});
if(!match)return null;
try{return JSON.parse(decodeURIComponent(match.slice(cookieName.length)))}catch(e){return null}
}
function hasChoice(){
return document.cookie.split(';').some(function(item){return item.trim().indexOf(cookieName)===0});
}
function loadGoogleAnalytics(){
if(analyticsLoaded||!googleMeasurementId)return;
...
if(choice&&choice.analytics===true)loadGoogleAnalytics();
if(!hasChoice())buildBanner();
}
